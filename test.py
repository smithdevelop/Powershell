# modified https://github.com/python/cpython/blob/3.7/Tools/parser/unparse.py
import sys
import ast
import tokenize
import io
...
class Unparser:
    ...
    def __init__(self, tree, file = sys.stdout, args_to_raise=[]):
        """Unparser(tree, file=sys.stdout) -> None.
         Print the source for tree to file."""
        self.f = file
        self._indent = 0
        self._args_to_raise = args_to_raise
        self.dispatch(tree)
        print("", file=self.f)
        self.f.flush()
    ...
    def _Name(self, t):
        if t.id in self._args_to_raise:
            self.write("try_raise(" + t.id + ")")
        else:
            self.write(t.id)
...
buf = io.StringIO()
args_to_check = ["a"]
Unparser(tree, buf, args_to_check)
buf.seek(0)
new_src=buf.read()
print(new_src)
exec(new_src)
